...
from src.dashboard.domain.interfaces.branches import ICompanyBranchGetDAO
...
class BranchGateway(ICompanyBranchGetDAO):
    pass
...
class WarehouseListUseCase(IWarehouseListUseCase):
    ...
    async def execute(self, branch_id: int) -> List[WarehouseEntity]:
        db_branch = await self._branch_gateway.get_by_id(id=branch_id)
        if not db_branch:
