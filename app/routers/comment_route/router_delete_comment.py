...
@router.delete("/animes/{anime_id}/comments/{comment_id}")
async def update_comment(anime_id: int, comment_id: int, db: Session = Depends(get_db),
    ...
    try:
        ...
        if db_anime:
            ...
            if db_comment:
                comment_repo.delete_comment(db, comment_id=comment_id, user_id=user.id,
                                            is_admin=user.is_superuser or user.is_moderator)
